...
type OnionForward struct {
	...
	// PortMap is a mapping from local ports bound to LocalAddr, to one or more
	// remote onion service ports.
	PortMap map[int][]int

	// LocalAddr is the local address from which ports are being forwarded to
	// the onion service. Default: 127.0.0.1
	LocalAddr string
...
func (t *Tor) Forward(ctx context.Context, conf *ForwardConf) (*OnionForward, error) {
	...
	fwd.LocalAddr, fwd.PortMap = conf.LocalAddr, conf.PortMap
	for localPort, remotePorts := range fwd.PortMap {
		...
		for _, remotePort := range remotePorts {
			req.Ports = append(req.Ports, &control.KeyVal{
				...
				Val: fwd.LocalAddr + ":" + strconv.Itoa(localPort),
			})
		}
	}
